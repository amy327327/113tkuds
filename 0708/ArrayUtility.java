
 import java.util.Arrays;
public class ArrayUtility {
   



    // o printArray(int[] array)：列印陣列內容
    static void printArray(int[] array) {
        System.out.println(Arrays.toString(array));
    }

    // o reverseArray(int[] array)：原地反轉陣列
    static void reverseArray(int[] array) {
        int left = 0;
        int right = array.length - 1;
        while (left < right) {
            // 交換元素
            int temp = array[left];
            array[left] = array[right];
            array[right] = temp;

            left++;
            right--;
        }
    }

    // o copyArray(int[] array)：建立副本
    static int[] copyArray(int[] array) {
        int[] copy = new int[array.length];
        for (int i = 0; i < array.length; i++) {
            copy[i] = array[i];
        }
        return copy;
    }

    // o findSecondLargest(int[] array)：找出第二大數值
    static int findSecondLargest(int[] array) {
        int max = Integer.MIN_VALUE;
        int secondMax = Integer.MIN_VALUE;

        for (int num : array) {
            if (num > max) {
                secondMax = max;
                max = num;
            } else if (num > secondMax && num < max) {
                secondMax = num;
            }
        }

        return secondMax;
    }

    // 主程式測試所有方法
    public static void main(String[] args) {
        int[] data = {3, 7, 1, 9, 4, 6, 8, 2, 5};

        System.out.println("原始陣列：");
        printArray(data);

        // 複製陣列
        int[] copy = copyArray(data);
        System.out.println("複製的陣列：");
        printArray(copy);

        // 找第二大數值
        int secondLargest = findSecondLargest(data);
        System.out.println("陣列中的第二大值是：" + secondLargest);

        // 原地反轉
        reverseArray(data);
        System.out.println("反轉後的原始陣列：");
        printArray(data);
    }
}

    

